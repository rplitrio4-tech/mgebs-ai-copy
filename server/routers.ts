...
export const appRouter = router({
  ...
  assistant: router({
    ...
    chat: publicProcedure.input(z.object({ messages: z.array(z.object({ role: z.enum(["user", "assistant"]), content: z.string() })), memory: z.array(z.string()).default([]), skills: z.array(z.string()).default([]), userName: z.string().optional(), sources: z.array(sourceSchema).default([]), deepThinking: z.boolean().default(true) })).mutation(async ({ input }) => {
      ...
      const response = await invokeLLM({ model: "gpt-5-mini", messages: [{ role: "system", content: context }, ...input.messages], reasoning: { effort: input.deepThinking ? "medium" : "low" }, maxTokens: 1800 });
      const raw = response.choices?.[0]?.message?.content; const content = typeof raw === "string" ? raw : "تمت معالجة طلبك."; const last = input.messages.at(-1)?.content || ""; const code = content.match(/```(?:([\w+-]+))?\n([\s\S]*?)```/); const memoryMatch = /(?:اسمي|أنا|عمري|أحب|أفضل)\s+([^،.!؟\n]{2,50})/i.exec(last);
      return { content: content.replace(/```(?:[\w+-]+)?\n[\s\S]*?```/g, "").trim() || content, thinking: `حلل الطلب وحدد المهارة المناسبة${input.sources.length ? ` · راجع ${input.sources.length} مصادر` : ""}${input.skills.length ? ` · استدعى ${input.skills.length} مهارات` : ""}`, memory: memoryMatch?.[0], artifact: code ? { title: "مخرج برمجي", language: code[1] || "txt", code: code[2].trim() } : undefined };
    }),
  }),
});
